...
- **Network:** Single SNMP agent on one UDP port (default 161), shared by all SNMP devices. Matches Modbus TCP pattern.
...
|--------|------|----------|---------|-------------|
| `oid` | VARCHAR(200) | NULL | — | Full SNMP OID string (e.g. `1.3.6.1.4.1.12345.1.1.0`) |

Alembic migration required. Modbus registers leave `oid` as null. SNMP registers fill `oid` and set `address` to 0 (or use for sort order).
...
New built-in SNMP template: **"UPS (SNMP)"** based on common UPS-MIB OIDs:

| Register | OID | Data Type | Unit | Description |
|----------|-----|-----------|------|-------------|
| input_voltage | 1.3.6.1.2.1.33.1.3.3.1.3.1 | float32 | V | Input Voltage |
| input_frequency | 1.3.6.1.2.1.33.1.3.3.1.2.1 | float32 | Hz | Input Frequency |
| output_voltage | 1.3.6.1.2.1.33.1.4.4.1.2.1 | float32 | V | Output Voltage |
| output_current | 1.3.6.1.2.1.33.1.4.4.1.3.1 | float32 | A | Output Current |
| output_power | 1.3.6.1.2.1.33.1.4.4.1.4.1 | uint32 | W | Output Power |
| battery_status | 1.3.6.1.2.1.33.1.2.1.0 | int16 | — | Battery Status (1-4) |
| battery_voltage | 1.3.6.1.2.1.33.1.2.5.0 | float32 | V | Battery Voltage |
| battery_temperature | 1.3.6.1.2.1.33.1.2.7.0 | float32 | °C | Battery Temperature |
| estimated_minutes_remaining | 1.3.6.1.2.1.33.1.2.3.0 | int32 | min | Estimated Minutes Remaining |
| estimated_charge_remaining | 1.3.6.1.2.1.33.1.2.4.0 | int32 | % | Estimated Charge Remaining |

These are from the standard RFC 1628 UPS-MIB.

### Built-in Simulation Profile

"Normal Operation" profile for UPS template:
...
- output_power: computed, voltage × current
...
When `protocol === "snmp"`, show an "OID" column instead of (or in addition to) "Address". The OID column renders a text input for the full OID string.

### TemplateForm

Protocol selector already exists. Add `"snmp"` to `PROTOCOL_OPTIONS`:
```typescript
