...
| File | Change |
|------|--------|
| `frontend/src/pages/Templates/TemplateForm.tsx` | Wrap content in Tabs: "Register Map" + "Profiles" (edit mode only) |
...
3. Options: each profile by name, plus a "None (no profile)" option
4. Pre-select the profile where `is_default === true`
5. Map selection to `profile_id` in the create request:
...
Batch creation tab gets the same dropdown with the same behavior.

### 2. ProfilesTab (Template Detail)

Renders inside the "Profiles" tab of TemplateForm (only shown in edit mode, not create mode).
...
**Behavior:**
- Built-in profiles: Edit opens modal with name/description editable, configs read-only. Delete is disabled.
- User profiles: Full edit and delete.
- Set as Default: calls `profileApi.update(id, { is_default: true })`. Only one default per template (backend enforces).
