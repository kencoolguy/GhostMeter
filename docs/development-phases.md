...
- [x] **Modbus client write detection (#71)**: read-only sim now records client writes (FC05/06/15/16) in a per-device in-memory ring buffer (accept-and-ignore, no persistence) — `write_tracker` singleton, `trace_pdu` recording, monitor-snapshot unread badge, `GET /write-events` (pure) + `POST /write-events/ack`, Monitor device-card badge + drawer. Lets EMS devs verify their system issued the right writes.
- [x] **#72 (BACnet)**: write detection extended to BACnet — `WriteProperty` recorded + acked (accept-and-ignore, no persistence) instead of `writeAccessDenied`; write-event model generalized (`operation` label + string values) so non-Modbus protocols fit. Reuses the shared ring buffer / API / UI.
- [x] **#72 (OPC UA)**: write detection extended to OPC UA — nodes made writable, client `Write` recorded (client's original value, even when type-coerced for apply) via a `PreWrite` server callback filtered to external sessions; value applied to the node (read-back friendly until next tick). Known follow-up: OPC UA writes not fault-gated + a write to a faulted node clears its fault callback.
...
- [x] **Per-protocol device slave_id limits** (2026-07-17): `DeviceInstance.port` is no longer a client-supplied field — the server derives it from the template's protocol (Modbus 502 / SNMP 10161 / OPC UA 4840 / BACnet 47808 / MQTT 1883 nominal), so the `(slave_id, port)` uniqueness check partitions by protocol instead of all 5 protocols sharing one pool of 247. Modbus/BACnet keep the 1-247 ceiling (real protocol field-width limits — 1-byte unit ID / VLAN MAC); SNMP/OPC UA/MQTT have no coded ceiling (they key devices by OID/node/topic, not slave_id) — practical limit is host memory / event-loop throughput. Frontend intentionally left unchanged (Slave ID input still caps at 247 in the UI); see dev log for rationale.
- [x] **Aggregate data mode (#95)** (2026-09-04): `data_mode="aggregate"` — main meter = Σ / avg / weighted_avg / max / min of named source devices' registers, `on_missing` policy with an engine-side last-known cache, aggregate-before-computed evaluation order, cycle rejection at API (422) and at device start (409). Structured params editor in the Simulation data-mode tab. Replaces the parameter-alignment workaround on the MVCB / PM-01..09 setup.
- [x] **#72 (SNMP) / #97** (2026-09-04): SET was silently dropped (no SetCommandResponder → client timeout). Fault-aware `SetCommandResponder` records each SET on a known OID into write-events (`Set`, register address, client value) and refuses with `notWritable`; exception fault → `genErr`.
- [x] **#98 stable OPC UA NodeIds** (2026-09-04): `ns=<idx>;s=<slave_id>` / `ns=<idx>;s=<slave_id>.<register_name>` instead of asyncua's creation-order numbering — survives restart and add/remove. **Breaking** for clients storing the old numeric ids.
- [ ] **#72 (remaining)**: MQTT command-topic (needs topic+payload design); plus the OPC UA write-fault-gating follow-up
