...
### Fixed
- **SNMP SET was silently dropped** (issue #97): the agent only registered Get/GetNext/GetBulk responders, so a SET request got no reply at all and the client timed out — indistinguishable from a dead device, and never visible in write-events. A fault-aware `SetCommandResponder` now records every SET on a known OID into the shared write-events buffer (`operation: "Set"`, `address` = the register's address, the client's value as text, resolved register name) and refuses the request with **`notWritable`** (the simulator stays read-only; the engine still owns the value). Under an `exception` fault SET answers `genErr` like reads; `timeout`/`intermittent` still go dark. Completes the SNMP part of #72 — only the MQTT command topic remains.

### Changed
- **Breaking (OPC UA): stable NodeIds** (issue #98). Nodes were created with only a namespace index, so asyncua auto-numbered them in creation order (`ns=2;i=1, 2, …`) and every backend restart or device add/remove shifted the ids, breaking clients that store NodeIds. Device objects are now `ns=<idx>;s=<slave_id>` and variables `ns=<idx>;s=<slave_id>.<register_name>` (e.g. `ns=2;s=10.breaker_command`); browse names / paths are unchanged. **Existing OPC UA client configurations must be updated to the new string NodeIds once**; they will not change again afterwards.

### Added
- **`aggregate` data mode — cross-device register aggregation** (issue #95): a register can now be derived from the same (or a named) register on other devices, so a main meter reads `Σ` of its sub-meters structurally instead of by hand-aligned parameters. `mode_params`: `op` (`sum` / `avg` / `weighted_avg` / `max` / `min`), `sources` (device names or ids), optional `register` (default: same name) and `weight_register` (weighted_avg), and `on_missing` (`last_known` default / `zero` / `skip`). The engine resolves sources once at launch, keeps a last-known value cache so a stopped/restarted sub-meter does not collapse the total, evaluates aggregate registers before the same device's `computed` ones, and refuses dependency cycles. The API rejects (422) unknown/ambiguous/self sources, a source template that lacks the register, and cycles across all devices (both `PUT` and `PATCH`); a cycle that slips through (e.g. via import) fails device start with `409` instead of silently running. UI: "Aggregate" in the Simulation data-mode selects plus a structured editor (multi-select source devices, op, register, weight register, on_missing) in place of the raw JSON textarea; the editor always submits device UUIDs (names in an existing config are mapped to ids on load) so renames or duplicate names never change what a saved config points at.

### Documentation
- Deployment guide: new **"搬移到另一個環境 (Migration)"** section — full procedure for moving an existing deployment (local or Linode) to a new host: `pg_dump` from the running postgres container, restore-before-deploy ordering (so Alembic only applies newer migrations and startup seeding skips existing builtin templates), carrying over the git-ignored `.env` (`POSTGRES_PASSWORD` / `BIND_IP` / `CLOUDFLARE_TUNNEL_TOKEN`), and post-move checklist for things not stored in the DB (new Tailscale node IP + re-sharing, Cloudflare Tunnel connector handover, MQTT broker source-IP allowlists).

### Added
- **Built-in template: DPM-C530 Three-Phase Power Meter** — Delta DPM-C520/C530 multifunction meter register map (32 float32 Holding Registers, FC03, block 0x100): per-phase + average phase/line voltages, per-phase + average currents, per-phase + total PF, frequency, per-phase + total kW/kVAR/kVA, and accumulated kWh/kVAh/kVARh. Ships with a default "Normal Operation" profile: gaussian phase voltages (~220V), line voltages derived as √3 × phase average, daily-curve currents peaking at 14:00, kW/kVA computed from V×I×PF, VAR derived from VA at PF 0.92, and accumulating energy counters.
- **Multi-broker MQTT publishing** (issue #87): brokers are now named, multi-row configuration (`GET/POST /api/v1/system/mqtt/brokers`, `PUT/DELETE /api/v1/system/mqtt/brokers/{id}`), and each device holds one publish config per broker with independent topic / interval / QoS / enabled state — so part of the fleet can feed one EMS pipeline while the rest feeds another. `MqttAdapter` keeps one client per broker with publish tasks keyed `(device_id, broker_id)`: one broker being down or reconnecting never touches other brokers' publishing (previously a settings update cancelled **every** device's publish task). Per-device start/stop take an optional `broker_id` (default: all configs; partial failures revert only that pair's enabled flag). Broker deletion is refused (`409 BROKER_IN_USE`) while device configs reference it. Migration renames the existing single broker row to `default` and re-attaches existing configs — deployed behavior is unchanged after upgrade. Export/import carries the broker list (`mqtt_brokers`, configs reference `broker_name`); legacy single-broker export files still import as `default`. UI: Settings shows a broker table (add/edit/test/delete + live connection state), the device page lists per-broker config rows with independent start/stop.

### Changed
- **Breaking (API):** `GET/PUT /api/v1/system/mqtt` (single global broker settings) removed in favor of the broker-list CRUD; per-device MQTT config endpoints now take a `broker_id` path segment (`PUT/DELETE /system/devices/{id}/mqtt/{broker_id}`) and `GET /system/devices/{id}/mqtt` returns a list. Import result field `mqtt_broker_settings_set` → `mqtt_brokers_set` (count).
...
- **Modbus client write detection** (issue #71): the read-only simulator now records client write attempts (FC05/06/15/16) into a per-device in-memory ring buffer (50 events) without persisting the written values — accept-and-ignore, so the simulation engine still overwrites on the next tick. Each write resolves to its template register name when the address matches a holding register. Surfaced as an unread badge on the Monitor device card (rides the existing 1Hz snapshot) plus a write-events drawer listing recent writes; backed by `GET /api/v1/devices/{id}/write-events` (pure list, newest first) and `POST /api/v1/devices/{id}/write-events/ack` (reset unread). Lets EMS developers verify their system issued the expected writes.
- **BACnet client write detection** (issue #72): the read-only simulator now records client `WriteProperty` requests against BACnet devices and **acknowledges success** (accept-and-ignore, like Modbus) instead of rejecting with `writeAccessDenied` — the written value is not persisted (the simulation engine still owns it). Writes go through the same comm-fault gate as reads, so a device under a timeout/intermittent fault records the attempt but still goes dark on the response. Reuses the shared write-events ring buffer / API / Monitor badge from #71.
- **OPC UA client write detection** (issue #72): the simulator's Variable nodes are now writable; client `Write` requests are recorded into the shared write-events buffer (the client's original value is logged, even when its Variant type is coerced to the node's type so the write applies) and the value is applied to the node — read-back friendly, then overwritten on the next simulation tick. Reuses the #71/#72 ring buffer / API / Monitor badge. **Known limitations (tracked as #72 follow-up):** OPC UA writes are not fault-gated (a device under a timeout/intermittent fault still accepts + applies writes, though the attempt is still recorded), and a client write to a node currently under a fault clears that node's fault value-callback (asyncua resets it on write), silently disabling the fault for that node until it is re-applied. Remaining write detection for SNMP / MQTT stays tracked in #72.

### Changed
- **Device `port` is server-derived, not client-supplied** (per-protocol slave_id limits): `DeviceCreate`/`DeviceBatchCreate`/`DeviceUpdate` no longer accept a `port` field — the server now derives it from the template's protocol (Modbus 502, SNMP 10161, OPC UA 4840, BACnet 47808, MQTT 1883 nominal). Previously every device defaulted to `port=502` regardless of protocol, so the `(slave_id, port)` DB uniqueness check accidentally pooled all 5 protocols into one shared 1-247 slave_id range. Now each protocol gets its own `(slave_id, port)` partition. `modbus_tcp` and `bacnet` keep a 247 ceiling on `slave_id` (real 1-byte protocol field limits: Modbus unit identifier, BACnet virtual-network MAC); `snmp`/`opcua`/`mqtt` no longer have any coded slave_id ceiling, since those adapters key devices by OID/node/topic and only used slave_id as a display label. Frontend unchanged for now (Slave ID input still shows a 247 max regardless of protocol).

### Fixed
- **MQTT brokers now auto-reconnect after a dropped connection**: aiomqtt has no built-in reconnect, and the adapter's `connected` flag was only ever set on connect — any TCP drop (broker restart, network blip, laptop broker going to sleep) left the client permanently dead while the API kept reporting `connected: true` and every publish failed with `[code:4]` (observed on Linode: both brokers down for 7+ hours until restart). A publish `MqttError` now flips the broker to disconnected (making status honest and silencing the per-publish error flood via the existing publish-loop guard) and starts a per-broker reconnect task with exponential backoff (1s → 60s cap); publish loops resume automatically on reconnect. A broker that is down at startup or after a settings update retries the same way instead of staying dead. Manual disconnect/reconnect/stop cancel the pending retry task.
- `test_health_returns_200` asserted a hardcoded version `0.1.0` (stale since 0.4.x); it now compares against `settings.APP_VERSION`.
- **MQTT publishing now survives backend restarts** (issue #84): the startup resume path only re-registered device runtimes, so every restart/deploy silently killed all publish loops while their configs stayed `enabled`. The resume block moved from the lifespan into `device_service.resume_running_devices()`, which now also calls `resume_enabled_publishing()` — enabled configs on running devices publish again right after startup.
- **MQTT broker settings now apply without a backend restart** (issue #81): `PUT /api/v1/system/mqtt` reconnects the running MQTT adapter with the saved (unmasked) settings — previously `MqttAdapter.reconnect()` was dead code and new settings only took effect after restarting the backend. Since a reconnect cancels every publish task, the route also restarts publishing for enabled configs on running devices after a successful reconnect.
- **`POST /api/v1/system/devices/{id}/mqtt/start` published to `gm/unknown`** (issue #82): the route started the publish loop without giving the adapter the device metadata, so topic templates rendered with fallback values (`device_name="unknown"`, `slave_id=0`) and the batch payload's `device` field was the raw UUID. The route now loads device name / slave ID / template name and calls `set_device_meta()` before `start_publishing()` — matching the device auto-start path, which was the only correct path before.

### Changed
- **Write-event model generalized** to fit any protocol: the Modbus-specific `function_code` (int) is now a human-readable `operation` label (e.g. `Write Register`, `WriteProperty`) and `values` are now strings (so a BACnet float present-value and a Modbus 16-bit word share one shape). Affects `GET /api/v1/devices/{id}/write-events` and the WebSocket monitor snapshot.
